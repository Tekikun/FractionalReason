...
class Evaluator:
    ...
    def get_exp_name(self, model_type, dataset_name, start_sample, n_samples, num_trials, alpha_mode, alpha_a, alpha_b, icv_mode):
        return f"{model_type.replace('/','-')}_{dataset_name.replace('/','-')}_{start_sample}_{n_samples}_icv_{icv_mode}_ntrials_{num_trials}_alpha_mode_{alpha_mode}_alpha_a_{alpha_a}_alpha_b_{alpha_b}"
    ...
    def run_evaluation(self, start_sample=0, n_samples=200, num_return_sequences=5, num_trials=20, alpha_a = 0, alpha_b = 0.15):
        ...
        exp_name = self.get_exp_name(self.gen_model_args['model_type'], self.ds_args['dataset_name'], start_sample, n_samples, num_trials, self.alpha_mode, alpha_a, alpha_b, self.icv_mode)
        output_file_path = os.path.join(self.root_path, exp_name + "_FA_majority_vote.json")
        print('=' * 20)
            ...
            with open(output_file_path, 'a' if os.path.exists(output_file_path) else 'w') as f:
                for query_id in tqdm(range(start_sample, end_sample), desc="Processing hard query"): 
                    ...
                    for _ in tqdm(range(num_trials), desc="Processing trials"):
                        ...
                        if alpha == 0:
                            generated_sentences = self.generate_without_icv(input_ids, num_return_sequences=num_return_sequences)
                        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--icv_mode', type=str, default='CoT', help='icv mode to use: CoT, knowledge, math, symbolic, graph, classifier')
