...
class BaseProbInference:
    ...
    def obtain_icv(model, inputs, rank=1):
        ...
        pca = PCA(n_components=rank, set_mean_to_zero=False).to(fit_data.device).fit(fit_data.float())
        variance = pca.explained_variance_ratio()
        # eval_data =  pca.transform(fit_data.float())
        # direction = pca.inverse_transform(eval_data).mean(0).view(hidden_states[demonstration_id][0].size(0), hidden_states[demonstration_id][0].size(1)) 
        ...
        sign_ = torch.sign(pca.components_.sum(dim=0,keepdim=True) @ (pca.mean_).T) # + pca.mean_
        direction = (pca.components_.sum(dim=0,keepdim=True) * sign_ + pca.mean_).mean(0).view(hidden_states[demonstration_id][0].size(0), hidden_states[demonstration_id][0].size(1))#h_pca.mean(0).view(hidden_states[demonstration_id][0].size(0), hidden_states[demonstration_id][0].size(1))
        return direction, variance#(neg_emb).view(hidden_states[demonstration_id][0].size(0), hidden_states[demonstration_id][0].size(1))

    def obtain_classifier(self, model, inputs, inputs_val):
        hidden_states = BaseProbInference.get_hiddenstates(model, inputs) #each element, layer x len_tokens x dim
        hidden_states_val = BaseProbInference.get_hiddenstates(model, inputs_val)
        num_demonstration = len(hidden_states)
        neg_all = []
        pos_all = []
        for demonstration_id in range(num_demonstration):
            neg_all.append(hidden_states[demonstration_id][0][-1])
            pos_all.append(hidden_states[demonstration_id][1][-1])
        
        # Stack all negative and positive examples
        neg_examples = torch.stack(neg_all)
        pos_examples = torch.stack(pos_all)
        
        # Create features and labels for logistic regression
        X = torch.cat([neg_examples, pos_examples], dim=0)
        means = X.mean(dim=0, keepdim=True)
        stds = X.std(dim=0, keepdim=True)

        # Standardize X
        X = (X - means) / stds
        y = torch.cat([torch.zeros(len(neg_all)), torch.ones(len(pos_all))])
        
        # Train logistic regression classifier
        from sklearn.linear_model import LogisticRegression
        
        # Prepare validation data
        num_val_demonstration = len(hidden_states_val)
        neg_val = []
        pos_val = []
        
        for demonstration_id in range(num_val_demonstration):
            neg_val.append(hidden_states_val[demonstration_id][0][-1])
            pos_val.append(hidden_states_val[demonstration_id][1][-1])
            
        neg_val_examples = torch.stack(neg_val)
        pos_val_examples = torch.stack(pos_val)
        
        X_val = torch.cat([neg_val_examples, pos_val_examples], dim=0)
        X_val = (X_val - means) / stds
        y_val = torch.cat([torch.zeros(len(neg_val)), torch.ones(len(pos_val))])
        
        # Search for best C value based on validation accuracy
        best_accuracy = 0
        ...
        for C in C_values:
            classifier = LogisticRegression(random_state=0, max_iter=1000, C=C)
            classifier.fit(X.cpu().numpy(), y.cpu().numpy())
            
            val_accuracy = classifier.score(X_val.cpu().numpy(), y_val.cpu().numpy())
        ...
        print(f"Best validation accuracy: {best_accuracy:.4f} with C={best_classifier.C}")
        classifier = best_classifier  # Use the best classifier
